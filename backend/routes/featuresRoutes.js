...
// ── Memory-only upload (Render has ephemeral disk — never use diskStorage) ────
const upload = multer({
  storage: multer.memoryStorage(),
  limits: { fileSize: 20 * 1024 * 1024 },
});

// ── Upload file to Cloudinary or fall back to base64 data URL ─────────────────
async function uploadFileToCloudinary(buffer, originalname, mimetype) {
  ...
  if (cloud_name && api_key && api_secret) {
    ...
    return new Promise((resolve, reject) => {
      cloudinarySDK.uploader.upload_stream(
        { folder: 'campus-connect/community', resource_type: 'auto', format: ext },
        (err, result) => err ? reject(err) : resolve(result.secure_url)
      ).end(buffer);
    });
  }
  // Fallback: base64 data URL stored in DB (no external dependency)
...
router.post('/community', authMiddleware, upload.single('file'), async (req, res) => {
  ...
  try {
    const fileUrl = await uploadFileToCloudinary(req.file.buffer, req.file.originalname, req.file.mimetype);
    const result = await pool.query(
      ...
      [req.user.userId, title, description, resource_type, subject_code, subject_name, department, parseInt(semester)||1, year_scheme, fileUrl, req.file.originalname]
    );
    res.status(201).json(result.rows[0]);
