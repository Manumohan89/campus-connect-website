        ...
        {loading ? <Box sx={{ display: 'flex', justifyContent: 'center', py: 6 }}><CircularProgress /></Box> : (
          <Box sx={{ overflowX: 'auto' }}>
            <Table size="small">
              ...
              <TableBody>
                {filtered.map(item => (
                  <TableRow key={item.id} sx={{ '&:hover': { bgcolor: 'var(--bg-card2,#F8FAFC)' } }}>
                    <TableCell sx={{ py: 1.25, maxWidth: 200 }}>
                      <Typography sx={{ fontWeight: 700, fontSize: '0.82rem', color: 'var(--text-1,#111827)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.title}</Typography>
                      {item.file_url && (
