...
import {
  ...
  TableRow, TableCell, TableBody, Tooltip, InputAdornment,
  LinearProgress
} from '@mui/material';
...
export default function AdminResources() {
  ...
  const handleFileUpload = async (e, resourceId) => {
    ...
    fd.append('file', file);
    try {
      const res = await adminApi.post(`/resources/${resourceId}/upload`, fd);
                  ...
                  {display.map(r => (
                    <TableRow key={r.resource_id} sx={{ '&:hover':{ bgcolor:'#F9FAFB' } }}>
                      <TableCell sx={{ maxWidth:240 }}>
                        <Typography sx={{ fontSize:'0.82rem', fontWeight:700, color:'var(--text-1,#111827)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{r.title}</Typography>
