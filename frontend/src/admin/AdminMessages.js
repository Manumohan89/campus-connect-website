          ...
          {loading ? <Box sx={{ display: 'flex', justifyContent: 'center', py: 6 }}><CircularProgress /></Box> : (
            <Box sx={{ overflowX: 'auto' }}>
              <Table size="small">
                ...
                <TableBody>
                  {filtered.map(m => (
                    <TableRow key={m.id} sx={{ '&:hover': { bgcolor: 'var(--bg-card2,#F8FAFC)' }, cursor: 'pointer' }} onClick={() => setViewMsg(m)}>
                      <TableCell sx={{ py: 1.5 }}>
                        <Typography sx={{ fontWeight: 700, fontSize: '0.85rem', color: 'var(--text-1,#111827)' }}>{m.name}</Typography>
                      </TableCell>
                      <TableCell>
      ...
      <Dialog open={!!viewMsg} onClose={() => setViewMsg(null)} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: '16px' } }}>
        ...
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <Box sx={{ p: 2, bgcolor: 'var(--bg-card2,#F8FAFC)', borderRadius: '10px', border: '1px solid #E2E8F0' }}>
