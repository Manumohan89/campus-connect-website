    ...
    if (barRef.current) {
      barChart.current = new Chart(barRef.current, {
        ...
        options: {
          ...
          plugins: {
            legend: { position: 'top', labels: { font: { family: "'Outfit',sans-serif", size: 12 }, color: '#374151', padding: 20 } },
    ...
    if (donutRef.current) {
      ...
      donutChart.current = new Chart(donutRef.current, {
        ...
        options: {
          ...
          plugins: {
            legend: { position: 'bottom', labels: { font: { family: "'Outfit',sans-serif", size: 12 }, color: '#374151', padding: 16, usePointStyle: true } },
            tooltip: { backgroundColor: '#1E293B', titleFont: { family: "'Syne',sans-serif", size: 13 }, bodyFont: { family: "'Outfit',sans-serif" }, padding: 12, cornerRadius: 10 },
          },
          cutout: '68%',
        },
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', minHeight:'100vh', background:'#F4F6FB', fontFamily:"'Outfit',sans-serif" }}>
      ...
      <main style={{ flex:1, maxWidth:1200, margin:'0 auto', padding:'32px 24px 60px', width:'100%' }}>
        ...
        {marks.length === 0 ? (
          <div style={{ textAlign:'center', padding:'80px 24px', background:'white', borderRadius:24, border:'1px solid #E8EDF5' }}>
            <div style={{ fontSize:'4rem', marginBottom:16 }}>📭</div>
            <h2 style={{ fontFamily:"'Syne',sans-serif", color:'#0F172A', fontSize:'1.2rem', fontWeight:800, marginBottom:8 }}>No marks uploaded yet</h2>
          ...
          <>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(150px,1fr))', gap:16, marginBottom:28, opacity:vis?1:0, transform:vis?'translateY(0)':'translateY(16px)', transition:'all 0.5s ease' }}>
              {[
                ...
                <div key={i} style={{ background:'white', borderRadius:18, padding:'20px 18px', border:`1.5px solid ${s.color}20`, boxShadow:`0 2px 16px ${s.color}10`, transition:'all 0.2s', cursor:'default', opacity:vis?1:0, transitionDelay:`${i*60}ms` }}
                  ...
                  <div style={{ fontWeight:700, color:'#374151', fontSize:'0.75rem', textTransform:'uppercase', letterSpacing:'0.07em' }}>{s.label}</div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr', gap:20, marginBottom:24, opacity:vis?1:0, transition:'all 0.5s ease 0.15s' }}>
              <div style={{ background:'white', borderRadius:22, padding:'24px', border:'1px solid #E8EDF5', boxShadow:'0 2px 16px rgba(0,0,0,0.04)' }}>
                <h3 style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:'0.9rem', color:'#0F172A', margin:'0 0 20px', display:'flex', alignItems:'center', gap:8 }}>
            ...
            <div style={{ background:'white', borderRadius:22, border:'1px solid #E8EDF5', overflow:'hidden', boxShadow:'0 2px 16px rgba(0,0,0,0.04)', opacity:vis?1:0, transition:'all 0.5s ease 0.3s' }}>
              <div style={{ padding:'18px 24px', borderBottom:'1px solid #F1F5F9', display:'flex', justifyContent:'space-between', alignItems:'center', background:'#FAFBFC' }}>
                <span style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, color:'#0F172A', fontSize:'0.9rem' }}>Subject Breakdown</span>
                {failed.length > 0 && (
                  ...
                  <thead>
                    <tr style={{ background:'#F8FAFC' }}>
                      {['#','Code','Subject','Int','Ext','Total','Credits','GP','Grade'].map(h => (
                        <th key={h} style={{ padding:'11px 14px', textAlign:'center', fontSize:'0.62rem', fontWeight:800, color:'#94A3B8', textTransform:'uppercase', letterSpacing:'0.08em', borderBottom:'1px solid #E8EDF5' }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {marks.map((m, i) => {
                      ...
                      return (
                        <tr key={i} style={{ borderBottom:'1px solid #F4F6FB', background:m.total<40?'#FFF8F8':'white', transition:'background 0.15s' }}
                          ...
                          <td style={{ padding:'11px 14px', textAlign:'left', color:'#374151', maxWidth:160 }}><span style={{ display:'block', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{m.subject_name||'—'}</span></td>
                          ...
                          <td style={{ padding:'11px 14px', textAlign:'center', fontWeight:700, color:'#374151' }}>{m.grade_points}</td>
