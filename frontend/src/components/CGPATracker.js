    ...
    <div style={{ display:'flex', flexDirection:'column', minHeight:'100vh', background:'var(--bg-page,#F4F6FB)', fontFamily:"'Outfit',sans-serif" }}>
      ...
      <main style={{ flex:1, maxWidth:1100, margin:'0 auto', padding:'32px 24px 60px', width:'100%' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))', gap:16, marginBottom:28, opacity:vis?1:0, transform:vis?'translateY(0)':'translateY(16px)', transition:'all 0.5s ease' }}>
          {[
            ...
            <div key={i} style={{ background:'var(--bg-card,white)', borderRadius:18, padding:'20px', border:`1.5px solid ${s.color}20`, boxShadow:`0 2px 16px ${s.color}10`, transition:'all 0.2s', cursor:'default', opacity:vis?1:0, transitionDelay:`${i*60}ms` }}
              ...
              <div style={{ fontWeight:700, color:'var(--text-2,#374151)', fontSize:'0.72rem', textTransform:'uppercase', letterSpacing:'0.08em' }}>{s.label}</div>
        ...
        {history.length === 0 ? (
          <div style={{ textAlign:'center', padding:'80px 24px', background:'var(--bg-card,white)', borderRadius:24, border:'1px solid var(--border,#E8EDF5)', marginBottom:24 }}>
            <div style={{ fontSize:'3.5rem', marginBottom:16 }}>📭</div>
            <h2 style={{ fontFamily:"'Syne',sans-serif", color:'var(--text-1,#0F172A)', fontSize:'1.1rem', fontWeight:800, marginBottom:8 }}>No semester history yet</h2>
            ...
            <div style={{ display:'flex', justifyContent:'center', gap:12, flexWrap:'wrap' }}>
              ...
              <button onClick={() => nav('/upload-marks')} style={{ padding:'12px 24px', borderRadius:12, border:'2px solid #E2E8F0', background:'var(--bg-card,white)', color:'var(--text-2,#374151)', fontFamily:"'Syne',sans-serif", fontWeight:700, cursor:'pointer' }}>📤 Upload Marks Card</button>
            </div>
          </div>
        ) : (
          <>
            {/* Chart */}
            <div style={{ background:'var(--bg-card,white)', borderRadius:22, padding:'28px', border:'1px solid var(--border,#E8EDF5)', boxShadow:'0 2px 16px rgba(0,0,0,0.04)', marginBottom:24, opacity:vis?1:0, transition:'all 0.5s ease 0.1s' }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20, flexWrap:'wrap', gap:12 }}>
                <h3 style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:'0.95rem', color:'var(--text-1,#0F172A)', margin:0 }}>📉 SGPA Trend</h3>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))', gap:14, marginBottom:24 }}>
              {[...history].sort((a,b)=>parseInt(a.semester)-parseInt(b.semester)).map((h, i) => {
                ...
                return (
                  <div key={h.id} style={{ background:'var(--bg-card,white)', borderRadius:18, padding:'20px', border:`1.5px solid ${color}25`, boxShadow:`0 2px 16px ${color}10`, position:'relative', transition:'all 0.2s', opacity:vis?1:0, transitionDelay:`${i*50}ms` }}
                    ...
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:14 }}>
                      <span style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:'0.8rem', color:'var(--text-2,#374151)' }}>Semester {h.semester}</span>
      ...
      {dialog && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.55)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:9999, backdropFilter:'blur(4px)', animation:'fadeIn 0.2s ease' }}>
          <div style={{ background:'var(--bg-card,white)', borderRadius:24, padding:'32px', width:'100%', maxWidth:400, boxShadow:'0 32px 80px rgba(0,0,0,0.3)', animation:'slideUp 0.25s ease' }}>
            <h2 style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, color:'var(--text-1,#0F172A)', fontSize:'1.1rem', margin:'0 0 24px', display:'flex', alignItems:'center', gap:10 }}>
            ...
            {[
              ...
              <div key={f.key} style={{ marginBottom:16 }}>
                <label style={{ display:'block', fontSize:'0.78rem', fontWeight:700, color:'var(--text-2,#374151)', marginBottom:6 }}>{f.label}</label>
            ...
            <div style={{ display:'flex', gap:12, marginTop:24 }}>
              <button onClick={() => setDialog(false)} style={{ flex:1, padding:'13px', borderRadius:12, border:'1.5px solid #E2E8F0', background:'var(--bg-card,white)', color:'var(--text-2,#374151)', fontFamily:"'Syne',sans-serif", fontWeight:700, cursor:'pointer' }}>Cancel</button>
