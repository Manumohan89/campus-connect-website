...
import {
  ...
  Alert, Chip, Snackbar, Grid, Paper, Divider
} from '@mui/material';
import { Notifications, Add, AccessTime, CheckCircle } from '@mui/icons-material';
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={5}>
            <Card elevation={0} sx={{ border: '1px solid #E5E7EB', borderRadius: '20px', p: 3, position: { md: 'sticky' }, top: { md: 80 } }}>
              <Typography sx={{ fontWeight: 800, fontSize: '1rem', color: 'var(--text-1,#111827)', mb: 0.5 }}>Add New Reminder</Typography>
                ...
                {pending.length > 0 && (
                  <>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <Typography sx={{ fontWeight: 700, color: 'var(--text-1,#111827)', fontSize: '0.95rem' }}>Upcoming ({pending.length})</Typography>
                    </Box>
                    {pending.map((r, i) => (
                      <Card key={i} elevation={0} sx={{ border: '1.5px solid #DDD6FE', borderRadius: '14px', p: 2.5, bgcolor: '#FAFAFF' }}>
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                          <Box sx={{ display: 'flex', gap: 1.5, flex: 1 }}>
                            ...
                            <Box>
                              <Typography sx={{ fontWeight: 700, color: 'var(--text-1,#111827)', fontSize: '0.9rem', mb: 0.25 }}>{r.message}</Typography>
