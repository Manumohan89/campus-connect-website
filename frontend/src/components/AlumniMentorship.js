        ...
        {alumni.skills?.length > 0 && (
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
            {alumni.skills.slice(0, 4).map((s, i) => (
              <Chip key={i} label={s} size="small" sx={{ bgcolor: 'var(--bg-card2,#F8FAFC)', border: '1px solid #E2E8F0', fontSize: '0.65rem' }} />
            ))}
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', bgcolor: 'var(--bg-card2,#F8FAFC)' }}>
      ...
      {connectDialog && (
        <Dialog open onClose={() => setConnectDialog(null)} maxWidth="sm" fullWidth>
          ...
          <DialogContent>
            <Box sx={{ display: 'flex', gap: 2, mb: 3, p: 2, bgcolor: 'var(--bg-card2,#F8FAFC)', borderRadius: 2 }}>
