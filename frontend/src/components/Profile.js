...
import {
  Edit, School, TrendingUp, BarChart, CameraAlt, Delete, Assignment, CalendarMonth, Notifications, Warning
...
function StatBox({ label, value, color, bg, icon }) {
  return (
    <Box sx={{ textAlign:'center', p:{ xs:1.5, sm:2 }, borderRadius:'16px', bgcolor: bg || '#F8FAFC', border:`1.5px solid ${color}22`, flex:1, minWidth:{ xs:'calc(50% - 8px)', sm:0 } }}>
...
export default function Profile() {
  ...
  // Handle Cloudinary URLs, base64 data URLs, and old relative paths
  const BACKEND = (process.env.REACT_APP_API_URL || '').replace('/api', '');
  const photoSrc = hasPhoto
    ? (profile.profile_avatar.startsWith('http') || profile.profile_avatar.startsWith('data:'))
      ? profile.profile_avatar
      : BACKEND + profile.profile_avatar  // old relative path -> prepend backend URL
    : null;
        ...
        <Container sx={{ position:'relative' }}>
          <Box sx={{ display:'flex', gap:3, alignItems:'flex-end', flexWrap:'wrap' }}>
            ...
            <Box sx={{ flex:1, minWidth:200 }}>
              <Typography sx={{ fontSize:{ xs:'1.35rem', sm:'1.75rem', md:'2.25rem' }, fontWeight:900, color:'white', fontFamily:"'Space Grotesk',sans-serif", lineHeight:1.1, wordBreak:'break-word' }}>
            ...
            {/* CGPA pill */}
            {cgpaNum > 0 && (
              <Box sx={{ textAlign:'center', bgcolor:'rgba(255,255,255,0.12)', backdropFilter:'blur(10px)', borderRadius:'16px', px:3, py:2, border:'1px solid rgba(255,255,255,0.15)', flexShrink:0, alignSelf:{ xs:'flex-start', sm:'auto' }, mt:{ xs:1, sm:0 } }}>
                ...
                <Box>
                  {marks.length === 0 ? (
                    ...
                    <Box sx={{ overflowX:'auto', WebkitOverflowScrolling:'touch',
                      ...
                      '&::-webkit-scrollbar-thumb':{ bgcolor:'#E5E7EB', borderRadius:99 } }}>
                          ...
                          {[...marks].sort((a,b) => (a.subject_code||'').localeCompare(b.subject_code||'')).map((m, i) => {
                            ...
                            return (
                              <tr key={i} style={{ borderBottom:'1px solid #F3F4F6', background: failed ? '#FFF5F5' : i%2===0 ? 'white' : '#FAFAFA' }}>
                                ...
                                <td style={{ padding:'12px 16px', color:'var(--text-2,#374151)', maxWidth:180, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{m.subject_name || m.subject_code}</td>
