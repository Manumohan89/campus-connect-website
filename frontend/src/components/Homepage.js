...
/* ─── Feature data ──────────────────────────────────────────────── */
const FEATURES = [
  { emoji:'📊', title:'SGPA / CGPA Calculator',    desc:'Upload any VTU marks card PDF — subjects auto-extracted, SGPA calculated instantly.',                tag:'Core',   color:'#6366F1', glow:'rgba(99,102,241,0.25)' },
  ...
  { emoji:'💻', title:'Coding Platform',            desc:'LeetCode-style problems with Python, Java, C, C++ compilers and real-time leaderboard.',               tag:'NEW',    color:'#10B981', glow:'rgba(16,185,129,0.2)' },
  ...
  { emoji:'🏆', title:'College Leaderboard',        desc:'Rankings from CGPA, coding solved, courses completed, attendance, and forum answers.',                  tag:'LIVE',   color:'#F59E0B', glow:'rgba(245,158,11,0.2)' },
  ...
  { emoji:'🏢', title:'Internship Programs',        desc:'Apply for curated internships. Earn Internship & Training Certificates on completion.',                 tag:'NEW',    color:'#4F46E5', glow:'rgba(79,70,229,0.22)' },
  { emoji:'📦', title:'Final Year Projects',        desc:'Browse ready-made projects with source code, or request a custom solution built just for you.',         tag:'NEW',    color:'#7C3AED', glow:'rgba(124,58,237,0.22)' },
  { emoji:'💰', title:'AI Data Earn Platform',      desc:'Complete micro-tasks (voice, image, text) and earn real ₹ credited to your wallet on approval.',       tag:'EARN',   color:'#10B981', glow:'rgba(16,185,129,0.22)' },
  { emoji:'🎯', title:'Aptitude Test Simulator',    desc:'TCS NQT, Infosys, Wipro, Accenture pattern mock tests with real-time scoring and topic analysis.',     tag:'NEW',    color:'#F59E0B', glow:'rgba(245,158,11,0.2)' },
];
...
export default function Homepage() {
  ...
  useEffect(() => {
    axios.get(`${PUBLIC_API_BASE}/users/platform-stats`).then(r => {
      const d = r.data;
      setStats({
  ...
  return (
    <div style={{ fontFamily: "'Outfit', sans-serif", background: 'var(--bg-page,#F6F8FC)', overflowX: 'hidden' }}>
      <PublicHeader />
      ...
      <section style={{
        ...
        <div style={{ position:'relative', zIndex:2, width:'100%', maxWidth:1200, margin:'0 auto', padding:'clamp(48px,8vw,80px) 20px clamp(60px,10vw,100px)' }}>
          ...
          <h1 style={{
            ...
            marginBottom:24,
            maxWidth:820 }}>
            {'Your VTU Academic'.split(' ').map((word, i) => (
              <span key={i} className="hero-word" style={{'--wi': i}} >{word}{' '}</span>
            ))}
            <span style={{
              ...
              {'Success Platform'.split(' ').map((word, i) => (
                <span key={i} className="hero-word" style={{'--wi': i + 4}}>{word}{' '}</span>
              ))}
            </span>
          </h1>
          ...
          {/* Stats row */}
          <div className="animate-fadeInUp delay-400" style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:12, maxWidth:480 }}>
            {[
      ...
      <div style={{ background:'var(--bg-page,#F6F8FC)', padding:'24px 0', overflow:'hidden', borderTop:'1px solid rgba(0,0,0,0.05)', borderBottom:'1px solid rgba(0,0,0,0.05)' }}>
        <div className="ticker-strip">
          {['📊 SGPA Calculator','💻 Coding Platform','🤖 AI Study Tutor','🏆 Leaderboard','📚 VTU Resources','💬 Peer Forum','🎓 Certificates','💼 Placements','🗂️ Flashcards','📰 VTU News','🎯 Interview Prep','💰 Earn Platform',
            '📊 SGPA Calculator','💻 Coding Platform','🤖 AI Study Tutor','🏆 Leaderboard','📚 VTU Resources','💬 Peer Forum','🎓 Certificates','💼 Placements','🗂️ Flashcards','📰 VTU News','🎯 Interview Prep','💰 Earn Platform',
          ].map((item, i) => (
            <span key={i} style={{ padding:'8px 18px', borderRadius:99, background:'white', border:'1px solid #E2E8F0', fontSize:'0.82rem', fontWeight:600, color:'#374151', whiteSpace:'nowrap', boxShadow:'0 1px 4px rgba(0,0,0,0.06)' }}>
              {item}
            </span>
          ))}
        </div>
      ...
      <section style={{ padding:'clamp(48px,8vw,96px) clamp(16px,4vw,24px)', background:'var(--bg-page,#F6F8FC)' }}>
        <Grid container spacing={3} sx={{ maxWidth: 1400, margin: '0 auto' }}>
          ...
          <div style={{ textAlign:'center', marginBottom:64 }}>
            ...
            <h2 style={{ fontFamily:"'Syne',sans-serif", fontSize:'clamp(1.8rem,4vw,3rem)', fontWeight:800, color:'var(--text-1,#0F172A)', letterSpacing:'-0.03em', marginBottom:12 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(clamp(150px,100%,280px), 1fr))', gap:20 }}>
            {FEATURES.map((f, i) => {
              const featureRoutes = {
                ...
                'Backlog Clearing Courses':  '/training',
                'AI Study Tutor':            '/ai-tutor',
                'Coding Platform':           '/coding',
                'VTU Resource Library':      '/vtu-resources',
                'Peer Doubt Forum':          '/forum',
                'Flashcard Decks':           '/flashcards',
                'College Leaderboard':       '/leaderboard',
                'Placement Command Centre':  '/placement-drives',
                'VTU News & Alerts':         '/vtu-news',
                'Resume Builder':            '/resume-builder',
                'Verified Certificates':     '/training',
                'Internship Programs':       '/internship-programs',
                'Final Year Projects':       '/projects',
                'AI Data Earn Platform':     '/earn',
                'Aptitude Test Simulator':   '/aptitude-test' };
              const path = featureRoutes[f.title];
              return (
              <div key={i} className="card-hover" onClick={() => path && navigate(path)}
                style={{
                ...
                <h3 style={{ fontFamily:"'Syne',sans-serif", fontSize:'1rem', fontWeight:700, color:'var(--text-1,#0F172A)', marginBottom:8, lineHeight:1.3 }}>{f.title}</h3>
      ...
      <section style={{ padding:'clamp(48px,8vw,96px) clamp(16px,4vw,24px)', background:'var(--bg-page,#F6F8FC)' }}>
        <div style={{ maxWidth:1100, margin:'0 auto' }}>
          <div style={{ textAlign:'center', marginBottom:56 }}>
            <h2 style={{ fontFamily:"'Syne',sans-serif", fontSize:'clamp(1.8rem,4vw,2.8rem)', fontWeight:800, color:'var(--text-1,#0F172A)', letterSpacing:'-0.03em', marginBottom:8 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(240px,1fr))', gap:20 }}>
            {TESTIMONIALS.map((t,i) => (
              <div key={i} className="card-hover" style={{
                background:'var(--bg-card,white)', borderRadius:20, padding:'28px 24px',
                ...
                <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                  ...
                  <div>
                    <div style={{ fontWeight:700, fontSize:'0.875rem', color:'var(--text-1,#0F172A)' }}>{t.name}</div>
      ...
      {/* ═══ FINAL CTA ══════════════════════════════════════════ */}
      <section style={{ padding:'clamp(48px,8vw,96px) clamp(16px,4vw,24px)', background:'var(--bg-card,white)', textAlign:'center' }}>
        <div style={{ maxWidth:640, margin:'0 auto' }}>
          <h2 style={{ fontFamily:"'Syne',sans-serif", fontSize:'clamp(1.8rem,4vw,2.8rem)', fontWeight:800, color:'var(--text-1,#0F172A)', letterSpacing:'-0.03em', marginBottom:16 }}>
      ...
      <style>{`
        ...
        .hero-word {
          display:inline-block;
          animation: word-reveal 0.55s cubic-bezier(.4,0,.2,1) forwards;
          animation-delay: calc(var(--wi, 0) * 75ms);
          opacity: 0;
        }
        @keyframes pulse-ring-hp {
        ...
        .live-ring {
          ...
          background:inherit;
          animation: pulse-ring-hp 1.8s ease-out infinite;
        }
        @keyframes ticker-hp {
        ...
        .ticker-strip {
          ...
          animation: ticker-hp 20s linear infinite;
        }
        .ticker-strip:hover { animation-play-state: paused; }
        .card-hover { transition: transform .25s ease, box-shadow .25s ease; }
        .card-hover:hover { transform: translateY(-5px); box-shadow: 0 20px 48px rgba(0,0,0,0.12) !important; }
