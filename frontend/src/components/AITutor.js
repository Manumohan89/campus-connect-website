...
import {
  ...
  CircularProgress, Alert, IconButton, Avatar, LinearProgress,
  Stack, Divider, Select, MenuItem, FormControl, InputLabel,
  Tooltip, Snackbar, List, ListItem, ListItemText, ListItemButton
} from '@mui/material';
                  ...
                  sessions.map(s => (
                    <Box key={s.id} onClick={() => loadSession(s.id)}
                      sx={{ px: 2, py: 1.25, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 1,
                            bgcolor: activeSession === s.id ? '#EEF2FF' : 'transparent', '&:hover': { bgcolor: '#F8FAFC' } }}>
              ...
              <Box sx={{ flex: 1, overflow: 'auto', p: 2.5 }}>
                ...
                {loading && (
                  <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'center', mb: 2 }}>
                    ...
                    <Box sx={{ bgcolor: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: '4px 18px 18px 18px', px: 2, py: 1.5 }}>
