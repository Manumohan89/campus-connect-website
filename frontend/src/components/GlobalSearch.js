...
export default function GlobalSearch({ onClose }) {
  ...
  const search = useCallback(async (q) => {
    ...
    try {
      ...
      const alumni    = (alumniRes.value?.data || []).slice(0, 2).map(a => ({
        title: a.full_name,
        path: `/alumni-mentorship`,
        type: 'alumni',
        desc: `${a.job_role || a.current_role || 'Mentor'} at ${a.current_company || 'Company'}`
      }));
    ...
    <Modal open onClose={onClose} sx={{ display:'flex', alignItems:'flex-start', justifyContent:'center', pt:{ xs:4, md:10 }, px:2 }}>
      <Paper elevation={24} sx={{
        ...
        <TextField
          ...
          InputProps={{
            startAdornment: <InputAdornment position="start"><SearchIcon sx={{ color: isDark ? '#94A3B8' : '#9CA3AF' }}/></InputAdornment>,
            endAdornment: loading ? <InputAdornment position="end"><CircularProgress size={18} /></InputAdornment> : null,
            disableUnderline: true,
            sx: { fontSize:'1rem', px:2, py:1.5, color: isDark ? '#F1F5F9' : '#111827' } }}
          variant="standard"
          sx={{
            '& .MuiInput-root':{ px:2, py:1.5, '&:before':{ display:'none' }, '&:after':{ display:'none' } },
          ...
          <>
            ...
            <Box sx={{ maxHeight:400, overflowY:'auto' }}>
              ...
              {['page','course','resource','placement','alumni'].map(type => {
                ...
                return (
                  <Box key={type}>
                    <Typography sx={{ px:2.5, py:1, fontSize:'0.7rem', fontWeight:700, color: isDark ? '#94A3B8' : '#9CA3AF', textTransform:'uppercase', letterSpacing:'0.1em', bgcolor: isDark ? 'rgba(148,163,184,0.08)' : '#F9FAFB' }}>{typeLabel}</Typography>
                    {group.map((r, ri) => {
                      const globalIdx = results.indexOf(r);
                      return (
                        <Box key={ri} onClick={() => goTo(r.path)}
                          sx={{
                            px:2.5, py:1.5, display:'flex', alignItems:'center', gap:2, cursor:'pointer',
                            bgcolor: globalIdx===selected ? (isDark ? 'rgba(79,70,229,0.22)' : '#EEF2FF') : (isDark ? '#0F172A' : '#FFFFFF'),
                            '&:hover':{ bgcolor: isDark ? 'rgba(79,70,229,0.15)' : '#F8F9FF' },
                            transition:'background 0.1s'
                          }}>
                          <Box sx={{ width:32, height:32, borderRadius:'8px', bgcolor: isDark ? 'rgba(148,163,184,0.12)' : '#F1F5F9', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                            {ICON[type]}
                          </Box>
                          <Box sx={{ flex:1, minWidth:0 }}>
                            <Typography fontWeight={700} fontSize="0.875rem" noWrap sx={{ color: isDark ? '#F1F5F9' : '#111827' }}>{r.title}</Typography>
                            {r.desc && <Typography variant="caption" noWrap sx={{ color: isDark ? '#94A3B8' : '#6B7280' }}>{r.desc}</Typography>}
                          </Box>
                          {globalIdx === selected && <KeyboardReturnIcon sx={{ fontSize:16, color: isDark ? '#94A3B8' : '#9CA3AF', flexShrink:0 }} />}
                        </Box>
                      );
                    })}
                  </Box>
        ...
        {query && !loading && results.length === 0 && (
          <Box sx={{ p:4, textAlign:'center', color: isDark ? '#94A3B8' : '#9CA3AF' }}>
        ...
        {!query && (
          <Box sx={{ px:3, py:2.5 }}>
            <Typography fontSize="0.75rem" fontWeight={700} color={isDark ? '#94A3B8' : '#9CA3AF'} mb={1} sx={{ textTransform:'uppercase', letterSpacing:'0.08em' }}>Quick navigation</Typography>
              ...
              {STATIC_PAGES.slice(0,8).map(p => (
                <Chip key={p.path} label={p.title} size="small" onClick={() => goTo(p.path)} clickable
                  sx={{
                    ...
                    '&:hover':{ bgcolor: isDark ? 'rgba(79,70,229,0.28)' : '#EEF2FF', color:'#4F46E5' }
                  }} />
              ))}
            </Box>
            <Typography variant="caption" sx={{ display:'block', mt:2, textAlign:'center', color: isDark ? '#64748B' : '#94A3B8' }}>↑↓ navigate · Enter to open · Esc to close</Typography>
          </Box>
        )}
      </Paper>
    </Modal>
