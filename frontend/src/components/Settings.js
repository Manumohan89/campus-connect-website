...
import {
  ...
  Divider, Stack, Chip, CircularProgress, Snackbar, LinearProgress,
  Switch, FormControlLabel, IconButton, InputAdornment,
...
export default function Settings() {
  ...
  const togglePushNotifications = async () => {
    if (!('serviceWorker' in navigator) || !('PushManager' in window)) {
      setSnack('Push notifications are not supported in this browser');
      return;
    }
    setPushLoading(true);
    try {
      ...
      if (pushEnabled) {
        ...
        if (sub) {
          ...
          await api.delete('/users/push-subscribe').catch(() => {});
        }
      ...
      setSnack('Could not enable push notifications: ' + e.message);
    }
  ...
  const confirmDelete = async () => {
    if (deleteConfirmText !== 'DELETE') {
      setSnack('Please type DELETE to confirm');
      return;
    }
        ...
        <SectionCard title="Push Notifications" subtitle="Get alerts for placement drives, exam dates and more" icon={<NotificationsIcon />}>
          ...
          {'Notification' in window && Notification.permission === 'denied' && (
            <Alert severity="warning" sx={{ mt:1.5, borderRadius:'10px', fontSize:'0.78rem' }}>
              Notifications blocked in browser settings. Go to Site Settings → Notifications → Allow.
            </Alert>
          )}
        </SectionCard>
