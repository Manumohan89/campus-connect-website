...
import { CloudUpload, Calculate, School, CheckCircle, Login } from '@mui/icons-material';
import { useNavigate, Link } from 'react-router-dom';
import PublicHeader from './PublicHeader';
import PublicFooter from './PublicFooter';
import axios from 'axios';   // raw axios — no auth token needed for public route
...
export default function SGPACalculator() {
  ...
  const handleCalculate = async () => {
    ...
    try {
      const res = await axios.post('/api/users/sgpa-public', fd, {
        ...
        <Grid container spacing={4}>
          ...
          <Grid item xs={12} md={7}>
                    ...
                    <TableHead>
                      <TableRow sx={{ bgcolor: '#F8FAFC' }}>
                        {['Code','Subject','Int','Ext','Total','Cr','GP','Grade'].map(h => (
                          <TableCell key={h} sx={{ fontWeight: 700, fontSize: '0.72rem', color: '#64748B', textTransform: 'uppercase', bgcolor: '#F8FAFC', py: 1.25 }}>{h}</TableCell>
                        ))}
                      </TableRow>
                    </TableHead>
                    <TableBody>
                      {result.subjects.map((s, i) => (
                        <TableRow key={i} sx={{ '&:hover': { bgcolor: '#F8FAFC' }, bgcolor: s.total_marks < 40 ? '#FFF5F5' : 'white' }}>
            ...
            <Card elevation={0} sx={{ border: '1px solid #E2E8F0', borderRadius: '20px', p: 3 }}>
              ...
              <Box sx={{ mt: 2.5, p: 2, bgcolor: '#F8FAFC', borderRadius: '10px', border: '1px solid #E2E8F0' }}>
