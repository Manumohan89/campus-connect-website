...
import {
  ...
  Chip, Grid, Paper, Snackbar, RadioGroup,
  FormControlLabel, Radio, Dialog, DialogTitle,
  DialogContent, DialogActions, Select, MenuItem, FormControl,
  InputLabel, IconButton, Tooltip
, Stack , Divider } from '@mui/material';
              ...
              return (
                <Card key={i} elevation={0} sx={{ border: `1.5px solid ${correct ? '#10B981' : '#EF4444'}44`, borderRadius: '14px', p: 2.5, bgcolor: correct ? '#F0FDF4' : '#FEF2F2' }}>
                  <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'flex-start', mb: 1.5 }}>
                    {correct ? <span style={{ color: '#10B981', marginTop: 2, flexShrink: 0, fontSize: '1.1rem' }}>✓</span> : <CancelIcon sx={{ color: '#EF4444', mt: 0.25, flexShrink: 0 }} />}
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', bgcolor: 'var(--bg-card2,#F8FAFC)' }}>
      ...
      <Container maxWidth="sm" sx={{ py: 5 }}>
        <Card elevation={0} sx={{ border: '1px solid #E5E7EB', borderRadius: '20px', p: { xs: 3, md: 4 } }}>
          ...
          {subject && (
            <Box sx={{ mb: 3, p: 2.5, bgcolor: 'var(--bg-card2,#F8FAFC)', borderRadius: '12px', border: '1px solid #E5E7EB' }}>
