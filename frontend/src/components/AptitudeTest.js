...
import {
  ...
  Dialog, DialogTitle, DialogContent, DialogActions, Stack, Snackbar, IconButton, Tooltip, Divider
} from '@mui/material';
...
export default function AptitudeTest() {
  ...
  const handleSubmit = useCallback((auto = false) => {
    stopTimer();
    // FIX: Only count questions where user actually selected an answer
    let correct = 0;
    const attempted = Object.keys(answers).length;
    questions.forEach((q, i) => {
      // answers[i] must exist (not undefined) AND match the correct answer
      if (answers[i] !== undefined && answers[i] !== null && answers[i] === q.a) correct++;
    });
    // If no questions were attempted, score is 0
    const finalCorrect = attempted === 0 ? 0 : correct;
    setScore({ correct: finalCorrect, total: questions.length, pct: attempted === 0 ? 0 : Math.round((finalCorrect / questions.length) * 100), attempted });
    setMode('result');
  ...
  const startTest = (co) => {
    ...
    setCompany(co);
    setQuestions(qs);
    setAnswers({});  // FIX: clear all previous answers
    setFlagged({});
    setCurrent(0);
    setScore(null);
      ...
      <Box sx={{ display:'flex', flexDirection:'column', minHeight:'100vh', bgcolor:'#F8FAFC' }}>
        ...
        <Container maxWidth="md" sx={{ py:5 }}>
          <Card elevation={0} sx={{ border:`2px solid ${grade.color}44`, borderRadius:'20px', p:{ xs:3, md:5 }, mb:4, bgcolor:grade.bg, textAlign:'center' }}>
            ...
            <Typography fontWeight={700} mb={0.5}>{score.correct}/{score.total} correct ({score.attempted || 0} attempted)</Typography>
            ...
            {questions.map((q, i) => {
              ...
              return (
                <Card key={i} elevation={0} sx={{ border:`1.5px solid ${correct ? '#10B981':'#EF4444'}44`, borderRadius:'14px', p:2.5, bgcolor: correct ? '#F0FDF4':'#FEF2F2' }}>
                  ...
                  <Box sx={{ pl:4 }}>
                    {q.o.map((opt, oi) => (
                      <Typography key={oi} fontSize="0.82rem" sx={{ color: oi===q.a ? '#065F46' : oi===answers[i] && !correct ? '#991B1B' : '#374151', fontWeight: oi===q.a||oi===answers[i] ? 700:400, mb:0.4 }}>
                        {oi===q.a ? '✓' : oi===answers[i] ? '✗' : '○'} {opt}
                      </Typography>
                    ))}
                    {q.e && (
                  ...
                  {questions.map((_, i) => (
                    <Grid item key={i}>
                      <Box onClick={() => setCurrent(i)} sx={{ width:34, height:34, borderRadius:'8px', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', fontSize:'0.72rem', fontWeight:700, border:'1.5px solid',
                        ...
                        color:       i===current ? '#4F46E5' : flagged[i] ? '#92400E' : answers[i]!==undefined ? '#065F46':'#9CA3AF' }}>{i+1}</Box>
                    </Grid>
                  ))}
                </Grid>
                <Divider sx={{ mb:1.5 }} />
