...
import {
  ...
  FormControl, InputLabel, LinearProgress, Snackbar
} from '@mui/material';
            ...
            <Card elevation={0} sx={{ border: '1.5px solid #E2E8F0', borderRadius: 3, overflow: 'hidden' }}>
              {questions.slice(0, 10).map((q, i) => (
                <Box key={q.id || i} onClick={() => { setCurrent(i); setAnswer(''); setEvaluation(null); setMode('practice'); }}
                  sx={{ px: 2, py: 1.5, borderBottom: '1px solid #F1F5F9', cursor: 'pointer', bgcolor: current === i && mode === 'practice' ? '#EEF2FF' : 'white', '&:hover': { bgcolor: 'var(--bg-card2,#F8FAFC)' } }}>
