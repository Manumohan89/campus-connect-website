...
import {
  ...
  Select, MenuItem, FormControl, InputLabel, Grid, LinearProgress, Tooltip
} from '@mui/material';
  ...
  return (
    <Box sx={{ mb:0.75 }}>
      <Box sx={{ display:'flex', justifyContent:'space-between', mb:0.3 }}>
        <Box sx={{ display:'flex', alignItems:'center', gap:0.5 }}>{icon}<Typography fontSize="0.68rem" color="text.secondary">{label}</Typography></Box>
        <Typography fontSize="0.68rem" fontWeight={700} color={color}>{Math.round(value)}</Typography>
      </Box>
      <LinearProgress variant="determinate" value={Math.min((value/max)*100,100)}
        sx={{ height:4, borderRadius:2, bgcolor:'#F1F5F9', '& .MuiLinearProgress-bar':{ bgcolor:color, borderRadius:2 } }} />
    </Box>
  );
}
