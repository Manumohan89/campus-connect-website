...
export default function UploadMarks() {
  ...
  const handleUpload = async () => {
    ...
    try {
      // Do NOT set Content-Type manually with FormData — axios sets it automatically
      // including the required multipart boundary. Manual override breaks multer parsing.
      const res = await api.post('/users/upload-marks', fd);
      setResult(res.data);
      ...
      <main style={{ flex:1, maxWidth:1200, margin:'0 auto', padding:'32px 24px 60px', width:'100%' }}>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:28, alignItems:'start' }}
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:20, opacity:vis?1:0, transform:vis?'translateY(0)':'translateY(20px)', transition:'all 0.5s ease' }}>
            ...
            {/* How it works steps */}
            <div style={{ background:'var(--bg-card,white)', borderRadius:22, border:'1px solid var(--border,#E8EDF5)', padding:'24px', boxShadow:'0 2px 16px rgba(0,0,0,0.04)' }}>
              <h3 style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:'0.9rem', color:'var(--text-1,#0F172A)', margin:'0 0 18px', display:'flex', alignItems:'center', gap:8 }}>
                ...
                {STEPS.map((s, i) => (
                  <div key={i} style={{ display:'flex', gap:14, alignItems:'flex-start', paddingBottom: i < STEPS.length-1 ? 18 : 0, position:'relative' }}>
                    ...
                    <div style={{ paddingTop:4 }}>
                      <div style={{ fontWeight:700, fontSize:'0.82rem', color:'var(--text-1,#0F172A)', marginBottom:2 }}>
            ...
            {/* VTU Grading scale */}
            <div style={{ background:'var(--bg-card,white)', borderRadius:22, border:'1px solid var(--border,#E8EDF5)', padding:'22px', boxShadow:'0 2px 16px rgba(0,0,0,0.04)' }}>
              <h3 style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:'0.88rem', color:'var(--text-1,#0F172A)', margin:'0 0 16px', display:'flex', alignItems:'center', gap:8 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:20, opacity:vis?1:0, transform:vis?'translateY(0)':'translateY(20px)', transition:'all 0.5s ease 0.1s' }}>
            ...
            {/* Upload card */}
            <div style={{ background:'var(--bg-card,white)', borderRadius:22, border:'1px solid var(--border,#E8EDF5)', padding:'28px', boxShadow:'0 2px 16px rgba(0,0,0,0.04)' }}>
              <h3 style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:'0.95rem', color:'var(--text-1,#0F172A)', margin:'0 0 20px' }}>Upload your PDF</h3>
              ...
              {loading && (
                <div style={{ marginBottom:18 }}>
                  <div style={{ display:'flex', justifyContent:'space-between', marginBottom:6 }}>
                    <span style={{ fontSize:'0.78rem', color:'var(--text-2,#374151)', fontWeight:600 }}>Processing your marks card…</span>
              ...
              <div style={{ background:'linear-gradient(135deg,#F0FDF4,#ECFDF5)', border:'2px solid #6EE7B7', borderRadius:22, padding:'24px', boxShadow:'0 8px 32px rgba(16,185,129,0.12)', animation:'slideUp 0.4s ease' }}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, marginBottom:18 }}>
                  {[['SGPA',result.sgpa,'#4F46E5'],['CGPA',result.cgpa,'#7C3AED']].map(([label,val,color]) => (
                    <div key={label} style={{ textAlign:'center', padding:'18px', background:'var(--bg-card,white)', borderRadius:14, border:`1px solid ${color}20` }}>
            ...
            {/* Current marks table */}
            {marks.length > 0 && (
              <div style={{ background:'var(--bg-card,white)', borderRadius:22, border:'1px solid var(--border,#E8EDF5)', overflow:'hidden', boxShadow:'0 2px 16px rgba(0,0,0,0.04)' }}>
                <div style={{ padding:'16px 22px', borderBottom:'1px solid var(--border,#F1F5F9)', display:'flex', justifyContent:'space-between', alignItems:'center', background:'#FAFBFC' }}>
                  <span style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, color:'var(--text-1,#0F172A)', fontSize:'0.88rem' }}>Current Marks</span>
                    ...
                    <thead>
                      <tr style={{ background:'#F8FAFC', position:'sticky', top:0 }}>
                        {['Code','Int','Ext','Total','Grade'].map(h => (
                          <th key={h} style={{ padding:'9px 12px', textAlign:'center', fontSize:'0.62rem', fontWeight:800, color:'#94A3B8', textTransform:'uppercase', letterSpacing:'0.08em', borderBottom:'1px solid var(--border,#E8EDF5)' }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
