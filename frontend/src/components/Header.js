...
const NAV_GROUPS = [
  ...
  { label:'Career', color:'#059669', items:[
    ...
    { label:'Internship Tracker', path:'/internship-tracker', icon:<WorkIcon        fontSize="small"/>, badge:'NEW' },
...
export default function Header() {
  ...
  useEffect(() => {
    ...
    api.get('/users/profile').then(r => {
      ...
      const avatarUrl = d.profile_avatar || null;
            ...
            <Collapse in={openGroup===group.label}>
              <List disablePadding sx={{ bgcolor:'#FAFAFA' }}>
                {group.items.map(item => (
                  <ListItem button key={item.path} onClick={() => { navigate(item.path); setMobileOpen(false); }} sx={{ pl:3, py:0.75, bgcolor: isActive(item.path)?`${group.color}12`:'transparent' }}>
        ...
        <Box sx={{ p:2 }}>
          ...
          <Box sx={{ px:2, py:1 }}>
            <Button fullWidth variant="outlined" onClick={toggleMode}
              ...
              sx={{ textTransform:'none', borderRadius:'10px', borderColor:'rgba(255,255,255,0.2)', color:'rgba(255,255,255,0.8)', justifyContent:'flex-start' }}>
          ...
          <Button fullWidth variant="outlined" onClick={() => { navigate('/notifications'); setMobileOpen(false); }}
            ...
            sx={{ textTransform:'none', borderRadius:2, mb:1, justifyContent:'flex-start' }}>
            Notifications {unreadCount > 0 && `(${unreadCount})`}
          </Button>
          <Button fullWidth variant="outlined" onClick={() => { navigate('/profile'); setMobileOpen(false); }} startIcon={<PersonIcon />} sx={{ textTransform:'none', borderRadius:2, mb:1, justifyContent:'flex-start' }}>Profile</Button>
          <Button fullWidth variant="outlined" onClick={() => { navigate('/settings'); setMobileOpen(false); }} startIcon={<SettingsIcon />} sx={{ textTransform:'none', borderRadius:2, mb:1, justifyContent:'flex-start' }}>Settings</Button>
